...
int main()
{
    ...
    for(int i = 0; i < no_of_queues; i++) scanf("%d", &length[i]);
    ...
    const int oo = 1e9;
    int best_queue, minimum = oo;
    for(int i = 0; i < no_of_queues; i++)
        if(rounds_to_arrive_at[i] < minimum)
            minimum = rounds_to_arrive_at[i], best_queue = i + 1;
